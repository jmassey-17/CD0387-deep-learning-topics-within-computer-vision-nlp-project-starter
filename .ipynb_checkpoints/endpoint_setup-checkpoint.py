...
def model_fn(model_dir):
    ...
    try:
        ...
        print('Model loaded as TorchScript successfully.')
...
def predict_fn(input_object, model):
    ...
    try:
        ...
        print(f"Input shape: {input_object.shape}, dtype: {input_object.dtype}")
        with torch.no_grad():
            prediction = model(input_object)
        logger.info('Prediction complete. Output shape: %s', prediction.shape)
        print('Prediction complete. Output shape: %s', prediction.shape)
        return prediction
    except Exception as e:
